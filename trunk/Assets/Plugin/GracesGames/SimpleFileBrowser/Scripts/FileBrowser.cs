    ...
    public class FileBrowser : MonoBehaviour
    {
        ...
        public void SetupFileBrowser(ViewMode newViewMode, string startPath = "")
        {
            ...
            if (uiCanvas != null)
            {
                ...
                _uiScript.actOnChangeLanguageTag += OnChangeLanguageTag;
            }
            else
            {
        ...
        public void SelectFile()
        {
            ...
            else
            {
                ...
                //DataMgr.Instance.LastLoadFile = _currentFile;

                SendFileSelectEvent(_currentFile);
            }
        }

        public void OnChangeLanguageTag(string val)
        {
            if (_mode == FileBrowserMode.Save)
            {
                string fileNameText = _uiScript.GetSaveFileText();
                fileNameText = fileNameText.Replace(".txt", "");

                var tmpSplit = fileNameText.Split('_');
                var tmpLanCode = "";
                if (tmpSplit.Length > 1 && Localization.IsLanguageCode(tmpSplit[tmpSplit.Length - 1]))
                    tmpLanCode = tmpSplit[tmpSplit.Length - 1];

                // if already has language tag
                if (tmpLanCode.Length > 0)
                    fileNameText = fileNameText.Replace("_" + tmpLanCode, "");

                fileNameText += ((val.Length > 0) ? "_" : "") + val;

                _uiScript.SetFileNameInputField(fileNameText, _fileExtensions[0]);
                UpdateFileBrowser();
            }
        }
        ...
        public void FileClick(string clickedFile)
        {
            ...
            if (_mode == FileBrowserMode.Save)
            {
                ...
                // set val of dropdown menu to default val 0
                _uiScript.SetValDropDownMenu(0);
            }
            else
            {
